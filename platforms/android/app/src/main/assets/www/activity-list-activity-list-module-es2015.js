...
let ActivityListPage = class ActivityListPage {
    constructor(router, dataShare, db, alert, auth) {
        this.router = router;
    ...
    ionViewWillEnter() {
        ...
        this.db.collection('categories', q => q.orderBy('name', 'asc')).snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["take"])(1)).subscribe(serverItems => {
            serverItems.forEach(a => {
                //this.hasChilds = true;
                let category = a.payload.doc.data();
...
//# sourceMappingURL=activity-list-activity-list-module-es2015.js.map